...
const BACKEND_URL = "https://api.theneuroledger.org";
...
export async function getAgentData(agentName: string): Promise<{ personality: string } | null> {
  try {
    ...
    console.error("Error fetching agent data from neuroledger api:", (error as Error).message);
    return null;
  }
}
...
export async function getAllAgents(): Promise<any[]> {
  try {
    ...
    console.error("Error fetching all agents from neuroledger api:", (error as Error).message);
    return [];
  }
}
